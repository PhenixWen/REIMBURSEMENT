        ...
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span>病种信息更改
                </div>
            ...
            <div class="tpl-block ">
                ...
                <div class="am-g tpl-amazeui-form">
                    ...
                    <div class="am-text-center" style="color: red">带黄色圆圈项为必填项</div>
							<br>
                        <form class="am-form am-form-horizontal" action = "/REIMBURSEMENT/BingZhongUpdate2Servlet" method = "post" onsubmit = "return check()">
                            <div class="am-form-group">
                                <label for="user-name" class="am-u-sm-3 am-form-label">病种编号</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-name" placeholder="输入病种编号" name = "diseId" value = "${requestScope.disease.diseId }" readonly="readonly">
                            ...
                            <div class="am-form-group am-form-warning am-form-icon am-form-feedback" id = "divComp">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" id="diseName" placeholder="输入病种名称" name = "diseName" value = "${requestScope.disease.diseName }">
...
<!--                                         <option value="泌尿生殖系统疾病">泌尿生殖系统疾病</option> -->
                                        <c:if test="${requestScope.disease.diseKind == '传染病' }">
								    		<option value="传染病" selected="selected">传染病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '传染病' }">
								    		<option value="传染病">传染病</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseKind == '心理病' }">
								    		<option value="心理病" selected="selected">心理病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '心理病' }">
								    		<option value="心理病">心理病</option>
								    	</c:if>
								    	 <c:if test="${requestScope.disease.diseKind == '肿瘤' }">
								    		<option value="肿瘤" selected="selected">肿瘤</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '肿瘤' }">
								    		<option value="肿瘤">肿瘤</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseKind == '心血管疾病' }">
								    		<option value="心血管疾病" selected="selected">心血管疾病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '心血管疾病' }">
								    		<option value="心血管疾病">心血管疾病</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseKind == '空调病' }">
								    		<option value="空调病" selected="selected">空调病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '空调病' }">
								    		<option value="空调病">空调病</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseKind == '精神病' }">
								    		<option value="精神病" selected="selected">精神病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '精神病' }">
								    		<option value="精神病">精神病</option>
								    	</c:if>
								    	 <c:if test="${requestScope.disease.diseKind == '消化系统疾病' }">
								    		<option value="消化系统疾病" selected="selected">消化系统疾病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '消化系统疾病' }">
								    		<option value="消化系统疾病">消化系统疾病</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseKind == '泌尿生殖系统疾病' }">
								    		<option value="泌尿生殖系统疾病" selected="selected">泌尿生殖系统疾病</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseKind != '泌尿生殖系统疾病' }">
								    		<option value="泌尿生殖系统疾病">泌尿生殖系统疾病</option>
								    	</c:if>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <select data-am-selected="{searchBox: 1, maxHeight: 100}" style="display: none;" name = "diseM">
                                        <c:if test="${requestScope.disease.diseM == '可报销' }">
								    		<option value="可报销" selected="selected">可报销</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseM != '可报销' }">
								    		<option value="可报销">可报销</option>
								    	</c:if>
								    	<c:if test="${requestScope.disease.diseM == '不可报销' }">
								    		<option value="不可报销" selected="selected">不可报销</option>
								    	</c:if>
								        <c:if test="${requestScope.disease.diseM != '不可报销' }">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="备注" name = "remarks" value = "${requestScope.disease.remarks }">
                                </div>
                            </div>
                            <div class="am-form-group">
