        ...
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span> 中心报销
                </div>
            </div>

            <div class="tpl-block">
                <div class="am-g">
                    <form action="/REIMBURSEMENT/zxbx_index" method="post" class="am-form am-form-inline">
                        <label for="doc-ipt-pwd-1">用户编号：</label>
                        <div class="am-form-group">
                            <input type="text" name="personId" id="doc-ipt-pwd-1" placeholder="编号">
                        </div>
                        <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
                        <label for="doc-ipt-pwd-1">用户姓名：</label>
                        <div class="am-form-group" >
                            <input type="text" name="personName" id="doc-ipt-pwd-1" placeholder="姓名">
                        </div>
                    ...
                    <div class="am-u-sm-12">
                        <form class="am-form">
                            <table class="am-table am-table-striped am-table-hover table-main">
                                <thead>
                                <tr>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">人员编号</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">姓名</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">身份号码</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">年度报销次数</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">年度费用总额</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">年度报销总额</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">封顶线</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">年度自费总额</th>
                                    <th style="padding-left:.3rem;padding-right:.1rem;" class="table-title">是否结算完毕</th>
                                    <th style="padding-left:4rem;padding-right:.1rem;" class="table-title">操作</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach items="${requestScope.ams}" var="a">
							    <tr>
							        <td>${a.person.personID}</td>
							        <td>${a.person.personName}</td>
							        <td>${a.person.certifiID}</td>
							        <td>${a.times}</td>
							        <td>${a.money}</td>
							        <td>${a.baoxiao}</td>
							        <td>${a.fd}</td>
							        <td>${a.cost}</td>
							        <c:if test="${a.done ne '未结算完毕'}">
							        <td>${a.done}</td>                        	
									</c:if>                                 
							        <c:if test="${a.done eq '未结算完毕'}">
							        <td style = "color : black;font-weight:bold;">${a.done}</td>                        	
									</c:if>
							        <td>
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <button type = "button" class="am-btn am-btn-default am-btn-xs am-hide-sm-only" onclick="window.open('/REIMBURSEMENT/msg_person?personId=${a.person.personID}')"><span class="am-icon-file-text-o"></span> 详情</button>
                                                <button type = "button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="window.open('/REIMBURSEMENT/msg_jz?personId=${a.person.personID}')"><span class="am-icon-pencil-square-o"></span> 编辑与报销</button>
                                            </div>
                                        </div>
                                    </td>
							    </tr>
                                    ...
                                    <ul class="am-pagination tpl-pagination">
	                                   <c:forEach begin="1" step="1" end="${requestScope.pages}" var="i">
										    ...
										    <c:if test="${i eq requestScope.pagenow }">
										    	<li class="am-active">
										    	<a href="/REIMBURSEMENT/zxbx_index?page=${i }">${i }</a>
										    	</li>
										    </c:if>
