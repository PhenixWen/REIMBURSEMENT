		...
		$(function(){
			$.ajax({
				...
				success:function(data){
					...
					$(data).each(function(i,h){
						if(h.feeKind == s)
							str += "<option selected = 'selected' value = " + h.feeKind + ">" + h.feeKind + "</option>";
						else
        ...
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span>药品信息更改
                </div>
            ...
            <div class="tpl-block ">
                ...
                <div class="am-g tpl-amazeui-form">
					<input type = "hidden" id = "hosrank"  value = "${requestScope.medicine.hosRank }">
					<input type = "hidden" id = "feekind"  value = "${requestScope.medicine.feeKind }">
                    <div class="am-u-sm-12 am-u-md-9">
                    	<div class="am-text-center" style="color: red">带黄色圆圈项为必填项</div>
						<br>
                        <form class="am-form am-form-horizontal" action = "/REIMBURSEMENT/YaoPinUpdate2Servlet" method = "post" onsubmit = "return check()">
                            <div class="am-form-group">
                                <label for="user-name" class="am-u-sm-3 am-form-label" >药品编号</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-name" placeholder="输入药品编号" name = "mediId" value = "${requestScope.medicine.mediId }" readonly="readonly">
                            ...
                            <div class="am-form-group am-form-warning am-form-icon am-form-feedback" id = "divComp">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" id="mediName" placeholder="输入药品名称" name = "mediName" value = "${requestScope.medicine.mediName }">
                            ...
                            <div class="am-form-group" id = "divComp2">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">最高限价</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="ceilingPri" placeholder="输入最高限价" name = "ceilingPri" value = "${requestScope.medicine.ceilingPri }">
                                </div>
                                <span id="spanComp2" ></span>
                            </div>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <select data-am-selected="{searchBox: 1, maxHeight: 100}" style="display: none;" name = "chargeRank" }">
                                        <c:if test="${requestScope.medicine.chargeRank == '一级' }">
								    		<option value="一级" selected="selected">一级</option>
								    	</c:if>
								        <c:if test="${requestScope.medicine.chargeRank != '一级' }">
								    		<option value="一级">一级</option>
								    	</c:if>
								    	<c:if test="${requestScope.medicine.chargeRank == '二级' }">
								    		<option value="二级" selected="selected">二级</option>
								    	</c:if>
								        <c:if test="${requestScope.medicine.chargeRank != '二级' }">
								    		<option value="二级">二级</option>
								    	</c:if>
								    	<c:if test="${requestScope.medicine.chargeRank == '三级' }">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <select data-am-selected="{searchBox: 1, maxHeight: 100}" style="display: none;" name = "examine"  }">
                                        <c:if test="${requestScope.medicine.examine == '需要审批' }">
								    		<option value="需要审批" selected="selected">需要审批</option>
								    	</c:if>
								        <c:if test="${requestScope.medicine.examine != '需要审批' }">
								    		<option value="需要审批">需要审批</option>
								    	</c:if>
								    	<c:if test="${requestScope.medicine.examine == '不需要审批' }">
								    		<option value="不需要审批" selected="selected">不需要审批</option>
								    	</c:if>
								        <c:if test="${requestScope.medicine.examine != '不需要审批' }">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="输入药厂名称" name = "factorName" value = "${requestScope.medicine.factorName }">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">国药准字</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="输入国药准字" name = "drugCertificate" value = "${requestScope.medicine.drugCertificate }">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">使用频次</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="输入使用频次" name = "useFrequency" value = "${requestScope.medicine.useFrequency }">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">用法</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="输入使用方法" name = "useMethod" value = "${requestScope.medicine.useMethod }">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">产地</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="输入产地" name = "place" value = "${requestScope.medicine.place }">
                                </div>
                            </div>
                            <div class="am-form-group" id = "divComp3">
                                <label for="user-weibo" class="am-u-sm-3 am-form-label">限定天数</label>
                                <div class="am-u-sm-9">
                                    <input type="text" id="dayy" placeholder="输入限定天数" name = "dayy" value = "${requestScope.medicine.dayy }">
                                </div>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" id="user-weibo" placeholder="备注" name = "remarks" value = "${requestScope.medicine.remarks }">
                                </div>
                            </div>
                            <div class="am-form-group">
